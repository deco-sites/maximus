...
export default function Form() {
  ...
  const handleSubmit: JSX.GenericEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      const subject =
        (e.currentTarget.elements.namedItem("subject") as RadioNodeList)?.value;
      const message =
        (e.currentTarget.elements.namedItem("message") as RadioNodeList)?.value;
      const state =
        (e.currentTarget.elements.namedItem("state") as RadioNodeList)?.value;
  ...
  return (
    ...
    >
      <div class="w-full mb-6 mt-7 flex flex-col">
        <label class="text-xs font-medium leading-[19px] tracking-[0.6px] text-neutral-800 uppercase">
          SEU NOME:
        </label>
        <input
          name="name"
      ...
      <div class="w-full flex justify-end">
        <button
          type="submit"
          disabled={loading}
          class="w-[200px] h-12 float-right flex items-center justify-center bg-[#171413] hover:opacity-80 text-xs font-medium leading-[19px] tracking-[0.6px] text-white transition-[0.3s] rounded-sm border-0"
        >
          Enviar
        </button>
      </div>
